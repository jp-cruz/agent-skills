...
|-------|-------------|---------|---------|
| [thoth-docker-setup](./thoth-docker-setup/) | Production-ready Docker Compose setup for Thoth with cross-platform support (macOS, Windows, Linux). Smart defaults with visibility: one-command setup (`./setup.sh`) detects system, recommends storage, and guides configuration. Includes automated environment detection, multi-provider LLM integration, storage intelligence, persistent volumes, and comprehensive troubleshooting. Works in any terminal (Claude Code CLI, Aider, VS Code, OpenCode, bash). | v0.6.1 | `npx skills@latest add jp-cruz/agent-skills/thoth-docker-setup` |
