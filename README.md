...
|-------|-------------|---------|---------|
| [row-bot-docker-setup](./row-bot-docker-setup/) | Production-ready Docker Compose setup for Row-Bot (formerly Thoth) with lessons learned from bare-metal migrations. Cross-platform support (macOS, Windows, Linux), smart defaults, one-command setup. Includes migration guide for existing Row-Bot users, automated environment detection, multi-provider LLM integration, security hardening, persistent volumes, and comprehensive troubleshooting. Works in any terminal. | v0.5.0 | `npx skills@latest add jp-cruz/agent-skills/row-bot-docker-setup` |
